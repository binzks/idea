...
<body>
	<div class="profile-info profile-user-info-striped">
		<c:forEach var="column" items="${columns}">
			<c:if test="${ empty powerColumns.get(column.name)}">
				<c:choose>
					<c:when test="${column.tag == 'TEXT'}">
						<div class="profile-info-row">
							<div class="profile-info-name">${column.describe }</div>
							<div class="profile-info-value">
								<c:if test="${empty data.get(column.name)}">&nbsp;</c:if>${data.get(column.name) }&nbsp;</div>
						</div>
					</c:when>
					...
					<c:when test="${column.tag == 'ITEM'}">
						<c:forEach var="item" items="${column.items }">
							<c:if test="${dataList.get(column.name) == item.key }">
								<div class="profile-info-row">
									<div class="profile-info-name">${column.describe }</div>
